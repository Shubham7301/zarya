...
class _SuperAdminMerchantsScreenState extends State<SuperAdminMerchantsScreen> {
  ...
  List<Merchant> get _filteredMerchants {
    ...
    if (_searchController.text.isNotEmpty) {
      ...
      merchants = merchants.where((merchant) {
        return merchant.businessName.toLowerCase().contains(searchTerm) ||
  ...
  Future<void> _toggleMerchantStatus(Merchant merchant) async {
    final adminProvider = Provider.of<SuperAdminProvider>(context, listen: false);
    final success = await adminProvider.toggleMerchantStatus(merchant.id!, !merchant.isActive);
    
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${merchant.businessName} ${merchant.isActive ? 'deactivated' : 'activated'} successfully'),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _DetailRow('Business Name', merchant.businessName),
